...
> 資產（`frontend/player/models/`），執行期只靠 `GLTFLoader` + `registry.json`，不需要 Blender。
> 原「第二階段 Blender 出版渲染」已從 roadmap 移除；出版級畫面也由 Three.js 負責。

### 碰撞物理（已模組化在 `frontend/player/lib/`，不要重新推導）
...
- 測試：`node --test frontend/player/lib/tests/*.test.js`（目錄形式會失敗，必用 glob）

### 驗證五件套（改完務必全跑）

```bash
node --test frontend/player/lib/tests/*.test.js                      # 期望 fail 0（todo 3 是已知缺口）
...
播放器由 `frontend/player/models/registry.json` 的 per-model `hide` 清單隱藏這類參考幾何，比對是
...
python3 backend/image_enhance.py \
...
# 3. 播放器零改碼：frontend/player/index.html?scene=<code>；跑 node tools/verify_scenes.mjs 驗收
```

只剩兩個必要人工判斷：**挑兩台 collider 的 track ID**、**標碰撞幀**（追蹤器碰前凍結，
無法自動判定）。可重現性**只有 `trajectory.json` 那半成立**（實測 sha256 位元組相同）。
`ground.png` 不可重現：`backend/output/` 在 gitignore、來源圖從未入庫，且
...
- **交付給標註的底圖＝使用者在 ① 當下看的那張**（2026-08-20 改）：前端把 `S.tab` 當
  ...
  量法：`backend/measure_genai_drift.py`（分塊相位相關）
...
（tainan_yongkang），真實影片用它會錯位——backend 的定位是合成場景的地面來源，
不是真實影片的地面來源。
...
- **座標對位＝用 G-projection 的參考衛星圖，不要用新擷取的 Google 圖**（2026-07-24 實證）：
  ...
  **陷阱**：`--sat-dir`（backend 新擷取的 Google 圖）是**另一張不同取景的圖**，
...
- **部署約束**：`scene-loader.js` 用 `../../scenes/` 相對路徑（player 在 frontend/ 下一層）
  → 站根必須同時含 `frontend/` 與 `scenes/`，只部署 `frontend/` 會全數 404
- **HD 底圖的幾何漂移已量化**（2026-08-17，分塊相位相關，同一張 sat_raw 同 prompt）：
  ...
  **仍未解**：兩家都是「重畫」不是「修圖」，`--input` 座標關鍵路徑照舊禁用 `--genai`。
  ...
  重量請用 `python3 backend/measure_genai_drift.py --code <code>`（比較時固定 `--tile`）。
...
- **換模型**：直接放新 GLB 到 `frontend/player/models/`、在 `registry.json` 補一筆（file/flip/hide），
...
2. **`center_box + z_cam=0` 的座標模型**（Codex 審查指出）：推論取 bbox 中心當地面接觸點，
   ...
   **bbox 備援定位同樣受此偏移影響**
3. 靜態部署（注意 `frontend/` 與 `scenes/` 必須同層、player 走 `../../scenes/`；Pages 未啟用）
