...
//      http://127.0.0.1:8946/frontend/player/index.html?scene=<code>，
...
async function waitForServer(timeoutMs = 10000) {
  ...
  while (Date.now() - t0 < timeoutMs) {
    try {
      const res = await fetch(`${BASE}/frontend/player/index.html`);
      if (res.ok) return;
    } catch {
      // server 還沒起來，繼續等
    }
...
async function verifyScene(browser, code) {
  ...
  try {
    ...
    await page.goto(`${BASE}/frontend/player/index.html?scene=${code}`,
