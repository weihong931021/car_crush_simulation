...
- [x] **衛星 pipeline 代號未驗證**：`code` 會變成 `output/<code>` 路徑，`../` 可寫到 output
      外。新增 `satellite_pipeline/common.py` 的 `validate_code`（`^[A-Za-z0-9_-]+$`，
...
**陷阱**：`--sat-dir`（satellite_pipeline 新擷取的 Google 圖）是另一張不同取景的圖，
...
- 註：`models/FSRCNN_x4.pb` 無任何**程式碼**引用（satellite_pipeline 五支 .py 全數零命中），
...
- 註：舊版 `trafficlab-project/scripts/{map_capture,image_enhance,pipeline_mapground}.py`
  （Esri 版）已被 `satellite_pipeline/` 取代
...
- 換車輛模型：放新 GLB 到 `threejs/models/`、`registry.json` 補 file/flip/hide；flip 在
